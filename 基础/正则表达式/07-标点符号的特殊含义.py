...
# m2 = re.search(r'f[a-d]m','pdsfbm')
# m2 = re.search(r'f[0-5]+m','pdsf4423m')
# m2 = re.search(r'f[0-5a-d]+m','pdsfcm')
m2 = re.search(r'f[0-5a-dx]+m', 'pdsfxm')  # 0<=中间值<=5 或者 a<=中间值<=d 或者 中间值=x
print(m2)

# | 用来表示或者 和 [] 有一定的相似，但是有区别
# [] 里的值表示的是区间，而且是单个字符
# | 就是可选值，可以出现多个值
print(re.search(r'f(x|prz|t)m', 'pasfprzm'))

# {} 用来限定前面元素出现的次数
# {n}:表示前面的元素出现 n 次
print(re.search(r'go{2}d', 'good'))
# {n,}：表示前面的元素出现n 次以上
print(re.search(r'go{2,}d', 'goooooood'))
# {,n}:表示前面的元素出现 n次以下
...
print(re.search(r'go*d', 'gooooooood'))
