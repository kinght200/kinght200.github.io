...
def get_page_sorce(url):
    resp = requests.get(url)
    resp.encoding = "gb2312"
    return resp.text


def parse_page_sorce(html):
    doc = PyQuery(html)
    ...
    for mt in mt_list:  # 拿到每一个mt
        ...
        time = mt("div > dl:nth-child(4) > dd").text()
        ...
        print(chexing, didian, time, jiage, youhao, gonglishu, other)


def main():
    url = "https://k.autohome.com.cn/146"
    # 1.提取页面源代码
    html = get_page_sorce(url)
    # 2.解析页面源代码，提取数据
    parse_page_sorce(html)
